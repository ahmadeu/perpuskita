                        ...
                        <div class="book-info mb-4">
                            <div class="row">
                                <div class="col-md-4">
                                    ...
                                    @else
                                        <div
                                            class="img-fluid rounded default-cover d-flex flex-column justify-content-center align-items-center">
                        ...
                        <form action="{{ route('user.borrowings.store') }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('user_notes') is-invalid @enderror" id="user_notes" name="user_notes"
                                    rows="3" placeholder="Tambahkan catatan jika diperlukan">{{ old('user_notes') }}</textarea>
                                @error('user_notes')
                            ...
                            <div class="mb-3">
                                <label for="pickup_time" class="form-label">Pilih Jam Pengambilan <span
                                        class="text-danger">*</span></label>
                                <select class="form-control @error('pickup_time') is-invalid @enderror" id="pickup_time"
                                    name="pickup_time" required>
                                    <option value="">Pilih jam pengambilan</option>
                                    @foreach ($pickupSlots as $time => $label)
                                        <option value="{{ $time }}"
                                            {{ old('pickup_time') == $time ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
